...
const exportFile = (filename, xnbObject) => {
    ...
    if (found) {
        ...
        switch (exported.type) {
            ...
            // SFD Effect
            case 'SFDEffect':
                buffer = toPNG(exported.width, exported.height, exported.data);
                extension = 'png';
                break;
            ...
            case 'SFDItem':
                ...
                buffer = JSON.stringify(exported.data, null, 4);
                extension = 'sfditem';
                break;
        }
...
function exportSFDItem(data, fileName) {
    const basename = path.basename(fileName, '.json');
    ...
    data.parts.forEach(({ textures, type }) => {
...
const resolveImports = filename => {
    ...
    if (found) {
        ...
        switch (ext) {
            case '.sfditem':
            {
                const data = fromSfdItem(exportedFile, dirname);
...
function fromSfdItem(exportedFile, dirname) {
    const data = JSON.parse(exportedFile.toString());

    for (const part of data.parts) {
        const { textures } = part
        for (let i = 0; i < textures.length; i++) {
            const textureFile = textures[i];
            if (textureFile == null) continue;

            const textureContent = fs.readFileSync(path.join(dirname, data.fileName, textureFile));
            textures[i] = fromPNG(textureContent);
        }
    }
    return data;
}
